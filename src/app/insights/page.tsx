...
interface Transaction {
  ...
  amount: number;
  type: "income" | "expense";
  date: string; // Ensure date is correctly typed
}
...
export default function InsightsPage() {
  ...
  const expenseTotals: Record<string, number> = transactions
    .filter((t) => t.type === "expense")
    .reduce((acc, t) => {
      acc[t.category] = (acc[t.category] || 0) + Math.abs(t.amount);
      return acc;
    }, {} as Record<string, number>); // ✅ Ensures type safety
      ...
      <div className="bg-gray-900 p-4 rounded-lg shadow-lg mt-6">
        ...
        {biggestExpenseCategory ? (
          <p className="text-white text-xl font-bold">
            {biggestExpenseCategory}: ₹{biggestExpenseAmount.toFixed(2)}
          </p>
        ) : (
      ...
      <div className="bg-gray-900 p-4 rounded-lg shadow-lg mt-6">
        ...
        {tips.length > 0 ? (
          ...
          <p className="text-gray-400 text-center">
            No financial tips at the moment. Keep up the good work!
          </p>
        )}
      </div>
    </div>
